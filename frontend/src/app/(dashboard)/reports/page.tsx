...
function StatBox({ icon, title, value, bg }: { icon: React.ReactNode; title: string; value: string; bg?: string }) {
  return (
    <div className={ounded-xl border p-5  dark:border-zinc-700 dark:bg-zinc-800}>
...
export default function ReportsPage() {
  ...
  const stats = useMemo(() => {
    const totalRevenue = invoices
      .filter((i: any) => i.invoice_type === "sales")
  ...
  const monthlyData = useMemo(() => {
    ...
    for (let i = 5; i >= 0; i--) {
      ...
      const rev = invoices
        .filter((inv: any) => {
          try { const id = new Date(inv.date); return inv.invoice_type === "sales" && id.getMonth() === mi && id.getFullYear() === yr; } catch { return false; }
        })
        .reduce((s: number, inv: any) => s + (Number(inv.total) || 0), 0);
      const exp = expenses
        .filter((e: any) => {
          try { const ed = new Date(e.date); return ed.getMonth() === mi && ed.getFullYear() === yr; } catch { return false; }
        })
  ...
  const exportCSV = () => {
    ...
    const a = document.createElement("a"); a.href = url; a.download = ${activeReport}-report-.csv; a.click(); URL.revokeObjectURL(url);
  };
  ...
  return (
    <motion.div variants={containerVariants} initial="hidden" animate="visible" className="space-y-6">
      <motion.div variants={itemVariants} className="flex items-center justify-between">
        <PageHeader title="Reports & Analytics" description={Data for } />
      ...
      <motion.div variants={itemVariants} className="flex gap-2 overflow-x-auto pb-2">
        {REPORT_TYPES.map((report) => (
          <button key={report.id} onClick={() => setActiveReport(report.id)}
            className={lex items-center gap-2 whitespace-nowrap rounded-lg border px-4 py-2 text-sm font-medium transition-colors }>
      ...
      {activeReport === "revenue" && (
        <>
          <motion.div variants={itemVariants} className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
            ...
            <StatBox icon={<FileText className="h-5 w-5 text-amber-600" />} title="Paid Invoices" value={${stats.paidInvoices} / } bg="bg-amber-50 dark:bg-amber-950" />
          </motion.div>
            ...
            <div className="h-72">
              <ResponsiveContainer width="100%" height="100%">
                <AreaChart data={monthlyData}>
                  ...
                  <YAxis className="text-xs" tick={{ fill: "currentColor" }} tickFormatter={(v) => fmt(v)} />
                  <Tooltip contentStyle={{ borderRadius: "0.5rem", border: "1px solid var(--border)", background: "var(--card)" }} formatter={(v: number) => [fmt(v), ""]} />
                  ...
                  <ResponsiveContainer width="100%" height="100%">
                    <RePie>
                      <Pie data={categoryData} cx="50%" cy="50%" innerRadius={55} outerRadius={90} paddingAngle={4} dataKey="value">
                        {categoryData.map((entry, index) => (<Cell key={cell-} fill={entry.color} />))}
                      </Pie>
                ...
                {invoiceStatusData.length > 0 ? (
                  <ResponsiveContainer width="100%" height="100%">
                    <BarChart data={invoiceStatusData}>
                      ...
                      <Tooltip contentStyle={{ borderRadius: "0.5rem", border: "1px solid var(--border)", background: "var(--card)" }} />
                      <Bar dataKey="value" name="Count" radius={[4, 4, 0, 0]}>
                        {invoiceStatusData.map((entry, index) => (<Cell key={cell-} fill={entry.color} />))}
                      </Bar>
                    </BarChart>
                  </ResponsiveContainer>
                ) : (<div className="flex h-full items-center justify-center text-sm text-gray-400">No invoice data yet</div>)}
              </div>
            </motion.div>
          </div>
        </>
        ...
        <>
          ...
          <div className="grid gap-6 lg:grid-cols-2">
            <motion.div variants={itemVariants} className="rounded-xl border bg-white p-6 dark:border-zinc-700 dark:bg-zinc-800">
              ...
              <div className="h-72">
                {employeeByDept.length > 0 ? (
                  <ResponsiveContainer width="100%" height="100%">
                    <RePie>
                      <Pie data={employeeByDept} cx="50%" cy="50%" innerRadius={55} outerRadius={90} paddingAngle={4} dataKey="count" nameKey="name">
                        {employeeByDept.map((entry, index) => (<Cell key={cell-} fill={entry.color} />))}
                      </Pie>
                      <Tooltip contentStyle={{ borderRadius: "0.5rem", border: "1px solid var(--border)", background: "var(--card)" }} />
                      <Legend verticalAlign="bottom" height={36} iconType="circle" iconSize={8} formatter={(value) => <span className="text-xs text-gray-500">{value}</span>} />
                    </RePie>
                  </ResponsiveContainer>
                ) : (<div className="flex h-full items-center justify-center text-sm text-gray-400">No employees yet</div>)}
              </div>
            </motion.div>
            <motion.div variants={itemVariants} className="rounded-xl border bg-white p-6 dark:border-zinc-700 dark:bg-zinc-800">
                  ...
                  <tbody>
                    {employees.length > 0 ? employees.map((e: any) => (
                      <tr key={e.id} className="border-b last:border-0">
                        ...
                        <td className="py-3"><span className={inline-flex items-center rounded-full px-2 py-1 text-xs font-medium }>{e.is_active ? "Active" : "Inactive"}</span></td>
                      </tr>
              ...
              {invoiceStatusData.length > 0 ? (
                <ResponsiveContainer width="100%" height="100%">
                  <RePie>
                    ...
                    <Tooltip contentStyle={{ borderRadius: "0.5rem", border: "1px solid var(--border)", background: "var(--card)" }} formatter={(v: number) => [${v}, "Invoices"]} />
                    <Legend verticalAlign="bottom" height={36} iconType="circle" iconSize={8} formatter={(value) => <span className="text-xs text-gray-500">{value}</span>} />
                  </RePie>
                </ResponsiveContainer>
                ...
                <tbody>
                  {invoices.length > 0 ? invoices.slice(0, 10).map((inv: any) => (
                    <tr key={inv.id} className="border-b last:border-0">
                      ...
                      <td className="py-3"><span className={inline-flex items-center rounded-full px-2 py-1 text-xs font-medium }>{inv.status}</span></td>
                    </tr>
