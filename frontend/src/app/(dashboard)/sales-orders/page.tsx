          ...
          {STATUS_PIPELINE.filter((s) => s.id !== 'cancelled').map((stage, i) => {
            ...
            return (
              <div key={stage.id} className="flex items-center">
                <motion.div whileHover={{ scale: 1.05 }} className="flex items-center gap-2 rounded-lg border bg-white px-3 py-2">
            ...
            <tbody>
              {orders.map((order: any) => (
                <tr key={order.id} className="border-b last:border-0 hover:bg-gray-50">
                  ...
                  <td className="p-4 text-gray-500">{formatDate(order.date ?? order.created_at ?? new Date())}</td>
