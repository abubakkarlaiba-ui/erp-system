            ...
            <tbody>
              {suppliers.map((s: any) => (
                <tr key={s.id} className="border-b last:border-0 hover:bg-gray-50">
                  ...
                  <td className="p-4"><span className="flex items-center gap-1.5"><Mail className="h-3.5 w-3.5 text-gray-500" />{s.email}</span></td>
                  ...
                  <td className="p-4 text-right">
                    <div className="flex items-center justify-end gap-1">
                      <button onClick={() => openEdit(s)} className="rounded-md p-1.5 hover:bg-gray-100"><Pencil className="h-4 w-4" /></button>
              ...
              {suppliers.length === 0 && <tr><td colSpan={7} className="p-8 text-center text-gray-500">No suppliers found</td></tr>}
            </tbody>
          </table>
        </div>
      </div>
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-50 flex items-center justify-center bg-black/60">
            <motion.div initial={{ scale: 0.95, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.95, opacity: 0 }} className="w-full max-w-lg rounded-xl border bg-white shadow-2xl p-6 mx-4">
              <div className="flex items-center justify-between mb-4">
                ...
                <button onClick={() => { setDialogOpen(false); setEditingSupplier(null); }} className="rounded-md p-1.5 hover:bg-gray-100"><X className="h-4 w-4" /></button>
              </div>
              <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
                ...
                <div>
                  ...
                  <input {...form.register('contactPerson')} className="mt-1 w-full rounded-md border bg-white px-3 py-2 text-sm" />
                ...
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    <input {...form.register('paymentTerms')} placeholder="e.g. Net 30" className="mt-1 w-full rounded-md border bg-white px-3 py-2 text-sm" />
                  </div>
                </div>
                <div>
                ...
                <div className="flex justify-end gap-3 pt-2">
                  <button type="button" onClick={() => { setDialogOpen(false); setEditingSupplier(null); }} className="rounded-md border px-4 py-2 text-sm hover:bg-gray-100">Cancel</button>
        ...
        {deleteConfirm && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-50 flex items-center justify-center bg-black/60">
            <motion.div initial={{ scale: 0.95, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.95, opacity: 0 }} className="w-full max-w-sm rounded-xl border bg-white shadow-2xl p-6 mx-4">
              ...
              <p className="text-sm text-gray-500 mb-4">Are you sure you want to delete this supplier?</p>
