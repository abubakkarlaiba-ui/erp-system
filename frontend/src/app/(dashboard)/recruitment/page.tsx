            ...
            <div className="space-y-3 min-h-[200px] bg-gray-100 rounded-xl p-3">
              {grouped[col.key]?.map((candidate, i) => (
                ...
                >
                  ...
                  <h4 className="font-medium text-black mt-3 text-sm">{candidate.name}</h4>
                  ...
                  <div className="flex items-center justify-between mt-3 pt-3 border-t border-gray-200">
                    <span className="text-[11px] text-gray-500">{format(parseISO(candidate.appliedDate), "MMM dd, yyyy")}</span>
            ...
            >
              <div className="flex items-start justify-between mb-6">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <h3 className="text-xl font-semibold text-black">{selectedCandidate.name}</h3>
              ...
              <div className="space-y-4">
                <div className="grid grid-cols-2 gap-4">
                  <div className="bg-gray-100 rounded-lg p-3">
                    ...
                    <p className="text-sm font-medium text-black">{selectedCandidate.email}</p>
                  </div>
                  <div className="bg-gray-100 rounded-lg p-3">
                    <p className="text-xs text-gray-500 mb-1">Phone</p>
                    <p className="text-sm font-medium text-black">{selectedCandidate.phone}</p>
                  </div>
                  <div className="bg-gray-100 rounded-lg p-3">
                    <p className="text-xs text-gray-500 mb-1">Experience</p>
                    <p className="text-sm font-medium text-black">{selectedCandidate.experience}</p>
                  </div>
                  <div className="bg-gray-100 rounded-lg p-3">
                    <p className="text-xs text-gray-500 mb-1">Education</p>
                    <p className="text-sm font-medium text-black">{selectedCandidate.education}</p>
                  </div>
                </div>
                <div className="bg-gray-100 rounded-lg p-3">
                  <p className="text-xs text-gray-500 mb-1">Applied Date</p>
                  <p className="text-sm font-medium text-black">{format(parseISO(selectedCandidate.appliedDate), "MMMM dd, yyyy")}</p>
                </div>
                <div className="bg-gray-100 rounded-lg p-3">
                  <p className="text-xs text-gray-500 mb-1">Notes</p>
                  <p className="text-sm text-black">{selectedCandidate.notes}</p>
                </div>
              </div>

              <div className="flex justify-end gap-3 mt-6">
                <button onClick={() => setSelectedCandidate(null)} className="px-4 py-2 text-black bg-gray-100 rounded-lg hover:bg-gray-100 transition-colors">
                  Close
                </button>
